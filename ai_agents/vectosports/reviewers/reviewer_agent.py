"""Reviewer agent - reviews analysis, grounds it with web search, and prepares final report."""
...
3. Use your web search tool to find scientific articles, studies, or expert opinions that support or add context to the analysis.

4. Synthesize the original analysis and the information from your research into a final, comprehensive report.

5. The final report must be formatted in Markdown (`.md`).

6. Always cite the sources used in your research within the final report.

7. Start the final report with the tag <!-- FINAL_REPORT_START -->.""",
            sub_agents=[]
        )

    async def run(self, payload):
        logger.info(f"Reviewer agent received payload: {payload}")
        result = await self.agent.run(payload)
        logger.info(f"Reviewer agent returned result: {result}")
        return result
