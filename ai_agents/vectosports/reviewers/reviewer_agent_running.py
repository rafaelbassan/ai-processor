"""Reviewer agent for running analysis - reviews analysis, grounds it with web search, and prepares final report."""

import logging

logger = logging.getLogger(__name__)

class ReviewerAgentRunning:
    def __init__(self):
...
Always respond in the language specified in the user message (e.g., 'Language: pt-br' for Portuguese). If not specified, default to English.""",
    ...
    async def run(self, payload):
        logger.info(f"Reviewer agent running received payload: {payload}")
        result = await self.agent.run(payload)
        logger.info(f"Reviewer agent running returned result: {result}")
        return result

reviewer_agent_running = ReviewerAgentRunning()
