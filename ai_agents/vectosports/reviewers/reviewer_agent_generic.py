"""Reviewer agent for generic analysis - reviews analysis, grounds it with web search, and prepares final report."""

import logging

logger = logging.getLogger(__name__)

class ReviewerAgentGeneric:
    def __init__(self):
...
Always respond in the language specified in the user message (e.g., 'Language: pt-br' for Portuguese). If not specified, default to English.""",
    ...
    async def run(self, payload):
        logger.info(f"Reviewer agent generic received payload: {payload}")
        result = await self.agent.run(payload)
        logger.info(f"Reviewer agent generic returned result: {result}")
        return result

reviewer_agent_generic = ReviewerAgentGeneric()
